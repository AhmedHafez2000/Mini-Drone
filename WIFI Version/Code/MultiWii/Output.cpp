...
void writeMotors() { // [1000;2000] => [125;250]
  ...
  motor[3] = (motor[3]-1000) << 1;
      ...
      #endif
    #endif
    #if defined(WIFI) // for wifi receiver, use pin 5 and 6 instead of 10 and 11
      #ifndef EXT_MOTOR_RANGE 
...
void initOutput() {
  ...
  /********  Specific PWM Timers & Registers for the atmega328P (Promini)   ************/
    ...
    #endif
    #if defined(WIFI)
      initializeSoftPWM(); // use pin 6,5 instead of 10,11 for wifi receiver
    #else
     #if (NUMBER_MOTOR > 1)
       TCCR1A |= _BV(COM1B1); // connect pin 10 to timer 1 channel B
     #endif
...
// It will also be used for wifi receiver (pin 11 is used for SPI so using 6 instead).

#if defined(WIFI) || ((NUMBER_MOTOR > 4) && (defined(PROMINI) || defined(PROMICRO)))
  ...
  void initializeSoftPWM(void) {
    #if !defined(PROMICRO)
      TCCR0A = 0; // normal counting mode
      #if defined(WIFI) || ((NUMBER_MOTOR > 4) && !defined(HWPWM6))
        TIMSK0 |= (1<<OCIE0B); // Enable CTC interrupt  
      #endif
      #if defined(WIFI) || (NUMBER_MOTOR > 6) || ((NUMBER_MOTOR == 6) && !defined(SERVO))
        TIMSK0 |= (1<<OCIE0A);
      #endif
    #else
      #if !defined(HWPWM6)
  ...
  #if defined(WIFI) || ((NUMBER_MOTOR == 6) && (!defined(SERVO) && !defined(HWPWM6)))
    ISR(SOFT_PWM_ISR1) { 
